...
export class AuthService {
  ...
  login(credentials: { email: string; password: string }): Observable<AuthResponse> {
    ...
    return this.http.post<AuthResponse>('http://18.191.193.40:9000/api/auth/login', credentials).pipe(
  ...
  register(payload: { firstname: string; lastname: string; password: string; email: string }): Observable<AuthResponse> {
    ...
    return this.http.post<AuthResponse>('http://18.191.193.40:9000/api/auth/register', payload).pipe(
      tap((response) => {
